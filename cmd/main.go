...
import (
	...
	"github.com/artemlive/jsm-operator/internal/client"
...
func init() {
	...
	utilruntime.Must(jsmv1beta1.AddToScheme(scheme))
...
func main() {
	...
	var jsmApiToken string
	var jsmUsername string
	var jsmGraphQLURL string
	var jsmOpsRestURL string
	...
	if jsmApiToken == "" {
		setupLog.Error(nil, "JSM API token is required")
		os.Exit(1)
	}
	...
	if jsmCloudID == "" {
		setupLog.Error(nil, "JSM Cloud ID is required, you can find it in your Atlassian Cloud URL, e.g. https://<your-cloud-id>.atlassian.net/_edge/tenant_info")
		os.Exit(1)
	}

	if jsmOpsRestURL == "" {
		setupLog.Error(nil, "JSM REST URL is required, the format should be https://api.atlassian.com/jsm/ops/api")
		os.Exit(1)
	}

	jsmOpsRestURL = fmt.Sprintf("%s/%s", jsmOpsRestURL, jsmCloudID)

	// Create a new JSM client with the provided configuration
	jsmClient, err := client.NewJSMClient(client.JSMConfig{
		GraphQLURL: jsmGraphQLURL,
		RestURL:    jsmOpsRestURL,
		Token:      jsmApiToken,
		Username:   jsmUsername,
		CloudID:    jsmCloudID,
	})
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "b1ad110d.macpaw.dev",
	...
	if err = (&controller.JSMServiceReconciler{
		Client:    mgr.GetClient(),
		Scheme:    mgr.GetScheme(),
		JSMClient: jsmClient,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "JSMService")
		os.Exit(1)
	}
	if err = (&controller.JSMTeamReconciler{
		Client:    mgr.GetClient(),
		Scheme:    mgr.GetScheme(),
		JSMClient: jsmClient,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "JSMTeam")
		os.Exit(1)
	}
